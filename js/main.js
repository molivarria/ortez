...
function queryDBUpdateForm(tx){
	...
	tx.executeSql('UPDATE agenda_curso SET nombre="'+$("#ti_nombre").val()+'", domicilio="'+$("#ti_domicilio").val()+'",telefono="'+$("#ti_telefono").val()+'",email="'+$("#ti_mail").val()+'",nota="'+$("#ti_nota").val()+'",categoria="'+cat+'",foto = "'+$.imageURL+'" WHERE id='+$.id);
}
...
function queryDBInsertForm(tx){
	...
	tx.executeSql("INSERT INTO agenda_curso (nombre,domicilio,telefono,categoria,foto,nota,email) VALUES ('"+$("#ti_nombre").val()+"','"+$("#ti_domicilio").val()+"','"+$("#ti_telefono").val()+"','"+cat+"','"+$.imageURL+"','"+$("#text_nota").val()+"','"+$("#ti_mail").val()+"')", [], newFormSuccess, errorDB);
}
